...
public class StudentDAO extends DAO implements StudentService{
	...
	public List<StudentVO> studentList() {
		...
		conn = getConn();
		try {
			psmt = conn.prepareStatement("SELECT * FORM student_test ORDER BY stud_no ASC");
	...
	public List<StudentVO> searchName(String name) {
		...
		conn = getConn();
		try {
			psmt = conn.prepareStatement("SELECT * FORM student_test WHERE stud_name LIKE '%'||?||'%' ORDER BY stud_no ASC");
	...
	public StudentVO searchNo(int number) {
		...
		conn = getConn();
		try {
			psmt = conn.prepareStatement("SELECT * FORM student_test WHERE stud_no=?");
	...
	public void delStudent(int number) {
		conn = getConn();
		try {
			psmt = conn.prepareStatement("DELETE * FORM student_test WHERE stud_no=?");
			psmt.setInt(1, number);
	...
	public void insertStudent(StudentVO student) {
		conn = getConn();
		try {
			psmt = conn.prepareStatement("INSERT INTO student_test VALUSE(?, ?, ?, ?)");
